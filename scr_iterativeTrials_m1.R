...
# -----------------------------------------------------------------------
# Function runs simulation and models data to recover parameters, and allows
#   you to iterate parameter values by setting the alternate parameter 
#   "scenarios" to override the baseline parameters. Scenarios only change
#   what is set in each list element of "scenarios" (doesn't run all permutations
#   of parameters set in scenarios object). It is good practice to time one
#   simulation run and estimate total time for all scenarios.
...
base_sim_params <- list(
  ...
  netting_mode = "random", netting_n_cells = 6, netting_days_per_week = 3
)
...
timing <- system.time({
  one_result <- run_parameter_sweep(base_sim_params, scenarios,
                                    fit_fun = fit_fun_factory, is_factory = TRUE, n_sims = 1)
})

one_result$summary %>% view()
print(timing)   # look at "elapsed" (seconds)

# rough extrapolation to the full sweep
seconds_per_trial <- timing["elapsed"]
n_scenarios <- 4   # however many you're planning
n_sims_per_scenario <- 10
...
#saveRDS(sweep, file = "~/Desktop/M1_simulation.rds")
